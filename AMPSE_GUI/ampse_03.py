...
# For poly regression:
#sar_adc_root = os.getcwd() + "/SAR_ADC_v2"
sar_adc_root = os.getcwd()
...
class TF_Model():
    ...
    def load_model(self,load_address,load_name=None,err_save =False):
        if not load_name:
            load_name = self.name
        all_data = load(open(sar_adc_root + "/" + load_address+"/init_"+load_name+".pkl", "rb" ))
        ...
        self.model(x)
        self.model.set_weights(load(open(sar_adc_root + "/" + load_address+"/model_"+load_name+".pkl", "rb" )))
        if err_save:
